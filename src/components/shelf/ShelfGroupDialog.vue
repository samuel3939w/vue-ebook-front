...
export default {
  ...
  computed: {
    ...
    categoryList() {
      const categoryList = [...this.defaultCategory, ...this.category];
      return categoryList.filter((item) => {
        if (
          (item.edit === 2 && this.isInGroup) ||
          item.edit !== 2 ||
          !item.edit
        ) {
          return true;
  ...
  methods: {
    ...
    createNewGroup() {
      //判斷分組名稱是否重複
      let groupNameRepeat=false;
      this.shelfList.forEach((book) => {
        if (book.type === 2 && book.title === this.newGroupName) {
          groupNameRepeat = true;
        }
      });
      if (groupNameRepeat) {
        this.simpleToast(this.$t("shelf.groupNameRepeat"));
        return;
      }
