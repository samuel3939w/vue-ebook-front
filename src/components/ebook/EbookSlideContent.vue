    ...
    >
      ...
      >
        <span
          ...
          >{{ item.label }}</span
        >
        <span class="slide-contents-item-progress"></span>
      </div>
    </scroll>
    <scroll
...
export default {
  ...
  methods: {
    async displaySearch(item, highlight = false) {
      await this.display(item.cfi, this.hideTitleAndMenu);
      if (highlight) {
    ...
    async search() {
      if (this.searchText && this.searchText.length > 0) {
        const result = await this.doSearch(this.searchText);
